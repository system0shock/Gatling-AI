...
Валидатор проверяет только содержимое и декларации уже существующих артефактов и
не пересчитывает хеши или байты; это делает детерминированный apply engine
непосредственно перед установкой.


Сразу после авторинга создаётся один точный patch и descriptor методики. Quality
 gate, validator, review, approval и apply используют тот же неизменённый patch и
descriptor и не создают его повторно.
