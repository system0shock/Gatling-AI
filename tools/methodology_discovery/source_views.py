...
def _inside(root: Path, candidate: Path, relative: str) -> Path:
    try:
        ...
        raise DiscoveryError(
            ...
            "source does not resolve to a file inside the confirmed repository",
            path=relative,
        ) from exc
    return resolved
...
class WorkingTreeSourceView(SourceView):
        ...
        def visit(directory: Path, prefix: PurePosixPath | None = None) -> None:
            ...
            for entry in entries:
                ...
                if entry.is_dir(follow_symlinks=False):
                    ...
                    continue
                if entry.is_symlink() and candidate.is_dir():
                    # Directory symlinks are deliberately not traversed, even internally.
                    continue
                if not entry.is_file(follow_symlinks=True):
                    continue
                resolved = _inside(self.repo_root, candidate, relative)
                try:
    ...
    def read_bytes(self, path: str, max_bytes: int) -> bytes:
        ...
        resolved = _inside(
            self.repo_root,
            self.repo_root / Path(*PurePosixPath(normalized).parts),
            normalized,
        )
        if resolved != record.resolved:
            raise DiscoveryError(
                "source-changed-during-scan", "source target changed after indexing", path=normalized
            )
        try:
            before = resolved.stat()
        except OSError as exc:
            raise DiscoveryError(
                "source-changed-during-scan", str(exc), path=normalized
            ) from exc
        if before.st_size != record.size or before.st_mtime_ns != record.mtime_ns:
            raise DiscoveryError(
                "source-changed-during-scan", "source metadata changed after indexing", path=normalized
            )
