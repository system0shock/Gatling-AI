...
def snapshot(**commits: str) -> dict:
    return {"snapshot_id": "f" * 64, "modules": {MODULES[key][0]: {"commit": value, "dirty": False, "kind": MODULES[key][1]} for key, value in commits.items()}}
...
def pages(versions: dict[str, int]) -> dict:
    return {"snapshot_id": "f" * 64, "pages": {page_id: {"version": version} for page_id, version in versions.items()}}
